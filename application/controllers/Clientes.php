...
class Clientes extends CI_Controller {
    ...
    public function add($cliente_id = NULL) {
        ...
        $this->form_validation->set_rules('cliente_email', '', 'trim|required|valid_email|max_length[50]|callback_check_email|is_unique[clientes.cliente_email]');
//          
        if (!empty($this->input->post('cliente_telefone'))) {
            $this->form_validation->set_rules('cliente_telefone', '', 'trim|max_length[14]|callback_check_telefone|is_unique[clientes.cliente_telefone]');
        }

        if (!empty($this->input->post('cliente_celular'))) {
            $this->form_validation->set_rules('cliente_celular', '', 'trim|max_length[15]|callback_check_celular|is_unique[clientes.cliente_celular]');
        }
            ...
            if ($this->form_validation->run()) {
                ...
                if($this->db->table_exists('contas_receber')){
                    if($cliente_ativo == 0 && $this->core_model->get_by_id('contas_receber', array('conta_receber_cliente_id' => $cliente_id, 'conta_receber_status' => 0))){
                        $this->session->set_flashdata('info', 'Este cliente não pode ser desativado, pois está sendo utilizado em <i class="fas fa-hand-holding-usd"></i>&nbsp;Contas a receber');
                        redirect('clientes');
                    }
                }
    ...
    public function check_celular($cliente_celular) {
        ...
        if ($this->core_model->get_by_id('clientes', array('cliente_celular' => $cliente_celular, 'cliente_id !=' => $cliente_id))) {
            $this->form_validation->set_message('check_telefone', 'Esse celular já existe');
            return FALSE;
        } else {
            return TRUE;
        }
